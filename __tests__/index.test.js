...
const school = "school";
const baseURL = `/WebUntis/jsonrpc.do?school=${school}`;

const getElementObject = (
  id = mockResponse.result.personId,
  type = mockResponse.result.personType
) => ({
  params: {
    options: {
      element: { id, type },
...
cases(
  ...
  async ({ validate }) => {
    ...
    mockAxios
      .onPost(baseURL, expect.objectContaining({ method: "getSchoolyears" }))
      .replyOnce(200, {
        result: [
          {
            id,
...
test("should getLatestSchoolyear throw error with empty array", async () => {
  ...
  await expect(() =>
    untis.getLatestSchoolyear(false)
...
cases(
  ...
  async ({ validate }) => {
    ...
    expect(await untis.getNewsWidget(new Date("11/13/2019"), validate)).toEqual(
      response
    );
  },
  [
...
test("should getNewsWidget catch invalid data", async () => {
  ...
  await expect(() =>
    untis.getNewsWidget(new Date("11/13/2019"))
  ).rejects.toThrowErrorMatchingSnapshot();
});
...
cases(
  ...
  async ({ validate }) => {
    ...
    mockAxios
      .onPost(
        baseURL,
        expect.objectContaining({ method: "getLatestImportTime" })
      )
      .replyOnce(200, { result: 123 });
...
cases(
  ...
  async ({ validate, postIndex }) => {
    ...
    expect(JSON.parse(mockAxios.history.post[postIndex].data)).toMatchObject(
      getElementObject()
    );
  },
  [
...
cases(
  ...
  async (validate) => {
    ...
    expect(JSON.parse(mockAxios.history.post[1].data)).toMatchObject(
      getElementObject(id, type)
    );
  },
  [
...
cases(
  ...
  async (validate) => {
    ...
    expect(
      await untis.getOwnTimetableForRange(dateStart, dateEnd, validate)
    ).toBe(123);
    expect(mockAxios.history.post[1].data).toMatch("20191113");
...
cases(
  ...
  async (validate) => {
    ...
    expect(
      await untis.getTimetableForRange(dateStart, dateEnd, id, type, validate)
    ).toBe(123);
    expect(mockAxios.history.post[1].data).toMatch("20191113");
    expect(mockAxios.history.post[1].data).toMatch("20191117");
    expect(JSON.parse(mockAxios.history.post[1].data)).toMatchObject(
...
cases(
  ...
  async (validate) => {
    ...
    expect(
      await untis.getOwnClassTimetableForRange(dateStart, dateEnd, validate)
    ).toBe(123);
    expect(mockAxios.history.post[2].data).toMatch("20191113");
...
cases(
  ...
  async ({ validate }) => {
    ...
    expect(mockAxios.history.get[0].url).toMatch("startDate=20191113");
    expect(mockAxios.history.get[0].url).toMatch("endDate=20191117");
  },
  [
    { name: "with validate", validate: true },
    { name: "without validate", validate: false },
...
cases(
  ...
  async ({ validate, response, data }) => {
    ...
    await expect(() =>
      untis.getHomeWorksFor(dateStart, dateEnd, validate)
    ).rejects.toThrowErrorMatchingSnapshot();
  },
  [
    {
...
cases(
  ...
  [
    ...
    { name: "getSubjects", method: "getSubjects", validate: false, post: 0 },
    { name: "getTeachers", method: "getTeachers", validate: true, post: 1 },
    { name: "getTeachers", method: "getTeachers", validate: false, post: 0 },
    { name: "getStudents", method: "getStudents", validate: true, post: 1 },
    { name: "getStudents", method: "getStudents", validate: false, post: 0 },
    ...
    { name: "getHolidays", method: "getHolidays", validate: false, post: 0 },
    { name: "getStatusData", method: "getStatusData", validate: true, post: 1 },
    {
      name: "getStatusData",
...
cases(
  ...
  async ({ validate }) => {
    ...
    expect(
      await untis.getHomeWorkAndLessons(dateStart, dateEnd, validate)
    ).toEqual({
      homeworks: {},
    });
    expect(mockAxios.history.get[0].url).toMatch("startDate=20191113");
...
cases(
  ...
  [
    ...
    { name: "date < 10 && mouth < 10", date: "09/08/2019", result: "20190908" },
  ]
);
