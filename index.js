...
class WebUntis {
     ...
     * @private
     */
    async _request(method, parameter = {}, validateSession = true, url = `/WebUntis/jsonrpc.do?school=${this.school}`) {
        if (validateSession && !await this.validateSession()) throw new Error("Current Session is not valid");
        const response = await this.axios({
            method: "POST",
            url: url,
...
class WebUntisSecretAuth extends WebUntis {
    ...
    async login() {
        ...
        if (response.data && response.data.error) throw new Error("Failed to login. " + (response.data.error.message || ""));
        if (response.headers && response.headers["set-cookie"] && this._getCookieFromSetCookie(response.headers["set-cookie"]) === false) throw new Error("Failed to login. Server didn't return a session id.");
        ...
        const person = find(webUntisLoginServiceUser.persons, {id: configResponse.data.data.loginServiceConfig.user.personId});
        ...
        try {
            ...
            if (typeof dayConfigResponse.data !== 'object' || typeof dayConfigResponse.data.data !== 'object') throw new Error();
...
class WebUntisQR extends WebUntisSecretAuth {
    ...
    constructor(QRCodeURI, identity) {
        ...
        super(uri.searchParams.get('school'), uri.searchParams.get('user'), uri.searchParams.get('key'), uri.searchParams.get('url'), identity);
    }
}
